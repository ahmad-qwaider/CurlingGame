...
function updateCanvasFromState(gameState) {
  ...
  // Recreate and add stones
  gameState.allStones.forEach(stoneData => {
      let stone = new Stone(stoneData.x, stoneData.y, stoneData.radius, stoneData.colour);
      stone.velocityX = stoneData.velocityX;
      stone.velocityY = stoneData.velocityY;
      stone.isMoving = stoneData.isMoving;
      allStones.add(stone);
      if (stone.colour === HOME_COLOUR) homeStones.add(stone);
      else visitorStones.add(stone);
  });

  gameState.shootingQueue.forEach(stoneData => {
      let stone = allStones.getCollection().find(s => s.x === stoneData.x && s.y === stoneData.y);
      if (!stone) {
          stone = new Stone(stoneData.x, stoneData.y, stoneData.radius, stoneData.colour);
          stone.velocityX = stoneData.velocityX;
          stone.velocityY = stoneData.velocityY;
          stone.isMoving = stoneData.isMoving;
          allStones.add(stone);
  ...
  // Update game flags and turn
  whosTurnIsIt = gameState.whosTurnIsIt;
  enableShooting = gameState.enableShooting;
